...
describe("Homepage-Organizer", () => {
  test("shows Loading… when no token (early return)", () => {
  ...
  test("fetches events with bearer token; renders empty-state when none", async () => {
    ...
    await waitFor(() =>
      expect(global.fetch).toHaveBeenCalledWith(
        "http://localhost:4000/v1/org_events",
  ...
  test("normalizes snake_case fields and displays them", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Minimum commitment:/i)).toBeInTheDocument();
      expect(screen.getByText(/5 hrs\/wk/i)).toBeInTheDocument();
  ...
  test("create event success (201 JSON): adds card and resets form", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Minimum time commitment/i), { target: { value: "3 hrs" } });
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Minimum commitment:/i)).toBeInTheDocument();
      expect(screen.getByText(/3 hrs/i)).toBeInTheDocument();
    });
  ...
  test("create event 204/no-body fallback uses randomUUID and still adds card", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Minimum time commitment/i), { target: { value: "2 hrs" } });
  ...
  test("create event 401 triggers alert and redirect to /User-login", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Minimum time commitment/i), { target: { value: "1 hr" } });
...
describe("Homepage-Organizer — extra branches", () => {
  ...
  test("Create Event early validations: minCommitment required", async () => {
    setupLocalStorageMock({
      access_token: "TKN",
      user: JSON.stringify({ username: "org1", email: "o@example.com" }),
    });
    ...
    // missing minCommitment
    fireEvent.change(screen.getByPlaceholderText(/^Location \*/i), { target: { value: "Here" } });
    fireEvent.change(screen.getByPlaceholderText(/Job description \*/i), { target: { value: "Details" } });

    fireEvent.click(screen.getByRole("button", { name: /Post Job/i }));
    expect(window.alert).toHaveBeenCalledWith("Minimum time commitment is required");
  });
  ...
  test("Closing the modal by clicking the backdrop", async () => {
    ...
    fireEvent.mouseDown(dialog.parentElement); // mousedown on backdrop
