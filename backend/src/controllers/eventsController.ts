...
// //Helper: Computes minutes from actions(signins & signouts) rows
...
//   //loop over all accepted actions/rows (ordered oldest → newest)
//   //Whenever u see sign_in, remember that time
//   //When u later see sign_out, you add (sign_out - lastSignIn) to totalMs
//   //At the end, if u r still signed in (lastSignIn != null), u add (now - lastSignIn) to cover the open interval.
...
//  *  - totalMinutes
//  *  - rules = simple toggle: if signed in -> canSignOut, else if event started -> canSignIn
//  */
...
// /*------------------------------------------------------------
// Build the JSON for each of the api endpoints and send it back
// --------------------------------------------------------------*/
...
//     if (!user) 
//       return res.status(401).json({ message: "Unauthorized" }); //??? should use errorClass to send 401 Unauthorized 
