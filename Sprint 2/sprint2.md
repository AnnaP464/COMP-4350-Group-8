...
### Sudipta Sarker
I am proud of this [commit](https://github.com/AnnaP464/COMP-4350-Group-8/commit/3d1dfbfec86f109d7e255a93d8338473f0d318c7): end-to-end “Create & List Events” feature that lets an Organizer create volunteer events which are immediately persisted and visible in the app. It touches the database, backend (routing, controller, service, validation, auth), API documentation, and frontend UI; all wired together with clear types and security checks. Coordinating schema -> validation -> controller -> service -> DB mapping -> docs -> UI was tricky. Ensuring auth works end-to-end (token -> middleware -> controller) and handling date/time rigorously were key pain points I solved cleanly. This commit also solves API validation: Added GET /v1/events which supports ?mine=1 to get events only created by you, and hardened JWT middleware (requireAuth) with better error paths, payload checks, and user lookup.
